...
.DESCRIPTION
  六步：
    ...
    5.  config       $CODEX_HOME/config.toml 補 [tools.update_plan] enabled = true（Codex 0.152 起預設關，流程的任務追蹤靠它）
                  先備份 config.toml.bak-<時間>，append 的那段用註解定界，-Uninstall 只拔那段
...
$ConfigBegin = '# bstack install-codex.ps1 加入（begin）'
$ConfigEnd = '# bstack install-codex.ps1 加入（end）'
...
function Run-Codex {
    <# 跑 codex CLI 的寫入類指令；-WhatIf 只印不跑。回傳 exit code。 #>
    param([string[]]$args_)
    $cmd = "codex $($args_ -join ' ')"
    if ($DryRun) { Write-Host "  [whatif] $cmd"; return 0 }
    Write-Host "  > $cmd"
    # 一定要 | Out-Host：不接的話 stdout 會變成回傳值、跟 exit code 混成陣列，呼叫端 -ne 0 永遠為真（install.ps1 踩過）
    & $script:CodexExe @args_ 2>&1 | Out-Host
    return $LASTEXITCODE
}
...
function Save-Manifest($m) {
    <# 寫 manifest（UTF-8、無 BOM）；-WhatIf 只印。 #>
    if ($DryRun) { Write-Host "  [whatif] 寫 manifest $ManifestPath"; return }
    New-Item -ItemType Directory -Path $CodexHome -Force | Out-Null
    [IO.File]::WriteAllText($ManifestPath, (($m | ConvertTo-Json -Depth 5) + "`n"), [Text.UTF8Encoding]::new($false))
}
...
function Invoke-Migrate {
    ...
    .DESCRIPTION 為什麼要清：Codex 對同名 skill 不合併、兩份都列給模型看（Task 0 用 codex debug prompt-input 實測，
      ...
      判定看內容不看檔名：目錄名在本 repo skills/ 清單內（動態讀目錄、不寫死）且 SKILL.md frontmatter `name:` 等於目錄名；
      同名但 name 不符的視為使用者自己的 skill、不動。
    ...
    if (Test-Path -LiteralPath $skillsRoot) {
        foreach ($name in $repoSkills) {
            ...
            if ($t -match "(?m)^name:\s*$([regex]::Escape($name))\s*$") { $targets += $d } else { $sameNameSkipped += $d }
        }
    }
    # AGENTS.md 只警告不動
    ...
    if ($sameNameSkipped.Count) { Write-Host "  （同名但 frontmatter name 不符、視為你自己的 skill、不動：$($sameNameSkipped -join '、')）" }
...
function Invoke-Uninstall {
    <#
    .SYNOPSIS 依 manifest 拆掉自己加的：codex plugin remove → 刪自己複製的 agents → config.toml 只拔定界那段 → 刪 manifest。
    ...
    if ($m.config_patched -and (Test-Path -LiteralPath $ConfigPath)) {
        ...
        $pattern = "(?ms)\r?\n?^$([regex]::Escape($ConfigBegin))\r?\n.*?^$([regex]::Escape($ConfigEnd))\r?\n?"
        if ($t -match $pattern) {
            if ($DryRun) { Write-Host "  [whatif] $ConfigPath 拔掉「$ConfigBegin」到「$ConfigEnd」那段（先備份）" }
            else { Backup-File $ConfigPath; Write-Utf8NoBom $ConfigPath ($t -replace $pattern, "`n"); Write-Host "  已從 $ConfigPath 拔掉 [tools.update_plan] 那段" }
        } else { Write-Host "  $ConfigPath 找不到定界註解（可能被手改過），不動它；請自行檢查 [tools.update_plan]" -ForegroundColor Yellow }
    }
...
for ($i = 1; $i -le $maxTry; $i++) {
    ...
    if ($rc -eq 0) { break }
    if ($i -lt $maxTry) { Write-Host "  codex plugin add 失敗（第 $i/$maxTry 次），3 秒後重試…" -ForegroundColor Yellow; Start-Sleep -Seconds 3 }
}
if ($rc -ne 0) {
    Write-Host "  codex plugin add $PluginId 連續 $maxTry 次失敗。若訊息是「failed to activate plugin cache entry: 存取被拒 (os error 5)」，那是 Windows 複製大樹後 rename 被拒；改用 -Source github（預設）樹小、較不易撞到，或稍後再跑一次本腳本。" -ForegroundColor Red
    exit 1
}
...
if (-not $manifest.PSObject.Properties['config_patched']) { $manifest | Add-Member -NotePropertyName config_patched -NotePropertyValue $false }
...
$hasOtherDef = $hasTable -or ($cfg -match '(?m)^\s*update_plan(\.enabled)?\s*=')
if ($alreadyOn) { Write-Host "  $ConfigPath 已有 tools.update_plan.enabled = true，不動" }
elseif ($hasOtherDef) {
    # 已有定義但不是 true：再 append 一個 [tools.update_plan] 會變 TOML 重複表、整個 config 讀不了，只能請使用者手改
    Write-Host "  $ConfigPath 已有 tools.update_plan 的設定但不是 enabled = true；為避免 TOML 重複表，本腳本不 append，請自行改成 enabled = true（任務追蹤靠它）" -ForegroundColor Yellow
} else {
    $block = "$ConfigBegin`n[tools.update_plan]`nenabled = true`n$ConfigEnd`n"
    $how = if (Test-Path -LiteralPath $ConfigPath) { "備份 $ConfigPath.bak-$RunStamp 後 append" } else { "建立 $ConfigPath 並寫入" }
    if ($DryRun) { Write-Host "  [whatif] ${how}：`n    [tools.update_plan]`n    enabled = true（以「$ConfigBegin」/「$ConfigEnd」定界）" }
    else {
        Backup-File $ConfigPath
        $sep = if ($cfg.Length -eq 0 -or $cfg.EndsWith("`n")) { '' } else { "`n" }
        if ($cfg.Length -gt 0 -and -not $cfg.EndsWith("`n`n") -and -not [string]::IsNullOrWhiteSpace($cfg)) { $sep += "`n" }
        New-Item -ItemType Directory -Path $CodexHome -Force | Out-Null
        Write-Utf8NoBom $ConfigPath ($cfg + $sep + $block)
