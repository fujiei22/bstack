...
具體**實作層級**安全檢查清單；security-audit 跑 STRIDE 找架構威脅，本 skill 找實作 bug，兩者互補。各主題 = ✅ PASS code 範例（有則附）+ verify checklist。
...
無 hand-off state。固定載入點只有 `security-audit` 使用契約第 7 步（T3 程式碼 diff 必用、純文件 diff 隨 audit 一起跳；T2 涉敏感情境可選）；其餘為 user 顯式呼叫。跑完該主題、產 finding 交呼叫方。
