...
2.6 **跑 §對外文字複查 的觸發判斷**（**全 tier 都跑**，與 2.5 共用同一個 `git diff`）。
...
**要防的事**：這一輪改到了給人讀的對外文字（README、release notes、使用文件），而**流程裡沒有任何一關會發現文案寫壞了**。這與 §漏網複查 是同一個形狀的問題——規則存在，但沒人在那個時點想起要用。
...
**排除 `docs/work/` 與 `docs/archive/`**——那是 spec / plan / review / 驗收記錄等**施工文件**，不是對外文字。不排除的話，每寫一份 plan 都會觸發它，而驗收關卡會被自己的施工文件淹掉。

**動作**：

1. 載入 `zh-humanize`，走它的「被 verify-done 自動載入」路徑
2. **只列清單，不問、不改、不停**——清單寫進 verify 結果就結束
3. 使用者要不要照著改，是他看完清單之後的事，不在本階段處理

**與 §漏網複查 刻意不同的一點**：`§漏網複查` 補判出大改會升 blocker；**本節不升 blocker**。

理由：設計語言有客觀依據（token 抽得到或抽不到、四項對齊檢查有沒有跑），文案好壞沒有。升 blocker 會讓每次改 README 都要回答一次，而多數改動是修錯字——**很快就會變成閃電點掠過的東西，那比沒有偵測更糟**，因為它會順便訓練人忽略這個位置的警告。

**界線**：本節只負責讓「這輪動了對外文字」這件事**看得見**。改不改、怎麼改，都不在 verify-done 做。
