...
每個視角 subagent 的 prompt 都含四段：**角色設定**、**標的性質**（必寫）、**檢視重點**、**回報格式 ＋ 怎麼把結論送回來**（必寫）。
...
兩欄都不屬於的標的（產出器、測試用例、schema、設定檔）**不要硬套最接近的那欄**，在「標的性質」段自己答「這個標的壞掉會長什麼樣」與「哪些問題在它身上不適用」。

> 為什麼不窮舉標的類型：窮舉永遠追不上新標的，逼派工者答一次「壞掉會怎樣」才追得上。不做會硬套最接近的那欄、問出一整批不適用的問題（實測 2026-09-03）

### 第 4 段：怎麼把結論送回來（必寫）

```
分析完成後，**把完整結論送回派工你的 agent**：Claude Code 用 `SendMessage`；Codex 沒有這個工具、派工者靠 `wait_agent` 收，寫在最終回覆裡就算送到。
...
**視角 Eng（架構 / 技術風險）— 必派（下限）**。你是工程架構 / 技術風險視角的 reviewer：
...
**回報格式**（純 markdown，無 preamble）：標題「## <視角名> 視角 review」，底下四節 ### Critical（必須處理）/ ### Major（強烈建議）/ ### Minor（可選）/ ### Nit（風格）。

**視角 DX（開發者體驗）— 有人要讀的東西時**。你是 Developer Experience 視角的 reviewer：
...
## Critical 共識（多視角同時提）
## Critical 各視角獨見（按視角分列）
## Major / Minor / Nit（去重後合併）
## 主 agent 建議
- 必處理：<列 critical>　建議處理：<列 major 中認同的>　略過：<列 minor / nit 中判定不影響的、附理由>
```
