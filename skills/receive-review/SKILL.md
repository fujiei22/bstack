...
5. **T3 全程**：即使「不危險類」也要 user 看 diff 才 commit（不強制 prompt、但顯式）。headless 時直接 commit（仍是一顆 commit），不落 diff 檔，PR body 列 fix commit 的 sha（PR 本身就有 diff）。
...
4. 印 `git diff HEAD~1` 給 user 看，不需 user 點頭；**T3 特例**：批次完成後、進下 phase 前，整個 diff 給 user 過一眼，有要 revert 的告知，否則進下 phase；headless 時直接進下 phase，fix commit 的 sha 由 finish-branch 列進 PR body
...
| 「T3 也偷偷 auto-fix 不告訴 user」 | T3 even 不危險 也給 user 看 diff；headless 例外見 `headless-mode`（直接 commit、sha 進 PR body） |
