...
> 為什麼要寫「完成後」：實測五個 subagent 全部只送 idle 訊號、得逐一去要才拿到結論，原因是沒人告訴它們要送；不寫會讓工作者把「做完」當「交了」。
...
**收工**：整合同 subagent 路線（§整合 / 衝突）；另確認每個隊友都已收工，用名字請 lead 送出關閉請求。
...
無依賴理論上無 conflict，但驗：`git status` clean；`git log <pre-group-sha>..HEAD --pretty="%h %s"` 看 N 個 commit 都進 branch；跑 group 範圍 test 一次（subagent 的 verify 各自跑，整合測保險）。

衝突 fallback（理論上不該發生）：
- 兩 subagent 改同檔同行 → conflict → 走 finish-branch §Conflict 流程
- 兩 commit 互相破壞（A 加 function、B 移它）→ parallel 標錯（T3 在 plan、T2 在施工清單 group 欄）→ 退 write-plan（T3）／ 交棒 brainstorm §補施工清單入口（T2）
...
2. 走 rules.md §Fail handling：**retry**（重 spawn、prompt 加「前次 fail 原因：<...>」）／ **adjust + retry**（主 agent 提 plan task 改動、user 點頭再 retry）／ **rollback**（`git reset --hard <pre-group-sha>` 整 group 重來）／ **退 execute-plan** 改串行 ／ **退 write-plan**（T3）或 **交棒 brainstorm §補施工清單入口**（T2）改 group 標
3. 不靜默 retry

## §結果整合 + hand-off state

完成（含 fail handling）後寫 state，控制權**還給 execute-plan**、推進下個 group：
...
| 「判定完直接開隊友」／「開關關著、悄悄退 subagent」（Claude Code）／「選單只寫好處」 | 禁；一律 `AskUserQuestion` 讓 user 選、講明開關狀態、每個選項寫代價；headless 例外見 `headless-mode` |
