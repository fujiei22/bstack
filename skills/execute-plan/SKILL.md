...
1. **讀 task 來源**：`plan_path` 有值（T3）→ Read plan.md。`plan_path` 為 null → Read `spec_path`：T2 取標題行**恰為** `## 施工清單` 的那張表、每列一個 task（`group` = parallel-group、「怎麼驗」= verify command）；T1 依 success criteria 自拆 task。T2 的 spec 沒這段、或超過 rules.md §Tier 表 的列數上限 → 交棒 brainstorm §補施工清單入口，不自己編。同時讀 `spec_path` 對齊目標。
   載入時宣告一句給 user 看：「Tier=T2：依 rules.md §Tier 表不寫 plan.md、不跑 review-plan；task 來源 = spec §施工清單（N 列）」。
2. **TaskCreate**：task 來源的每個 task / 每一列落到 TaskCreate（含 parallel-group 屬性）。
3. **逐 group 推進**：同 `parallel-group` 多 task → 載 `dispatch-parallel`、由它判跑法（Agent Teams / subagent / 串行）並問 user；單 task group → 主 agent 自己跑 tdd-cycle。
...
**禁止**：跳 task / 重排序（除非升級為 user gate）、跳 tdd-cycle 紅綠循環、多 task 累一個大 commit。
...
**禁止**：發現前端檔只在心裡記一下就繼續寫；把中途轉進當 §Blocker 停在那裡（轉進**有明確回歸點**，是繞路不是撞牆）；為了不轉進把前端改動拆到別的 branch 偷做（選項 4 指**本 branch 內**的新 task）。
...
多 task group（1、3）：載 dispatch-parallel → spawn subagent 跑 A、B、自己跑 C → 等全部 done（subagent 回報 + 自己 verify 過）→ 整合、確認沒衝突 → 推進下個 group。單 task group（2）：主 agent 直接跑 tdd-cycle。

**重要**：subagent 完成 task 後**不直接 commit**；由主 agent 收結果、整合確認、最後統一 commit。

## §Verify 規則

每 task `commit` step 之前：task 內指定的 verify command（跑、印 output、對 expected）→ 既有 test suite 最小子集（動到的模組周邊）→ lint / type-check 該跑就跑。非綠 → 停下、進 §Task fail 處置。
...
**禁**：subject 寫英文（除非有 user 明確指示）、一個 commit 含多個 task、skip pre-commit hook（`--no-verify`）。

## §Blocker

**立即停下**並提 user：缺 dependency / 環境 / 設定、plan step 與 codebase 現狀牴觸（API 已變、檔已搬）、task 指令不清、verify 反覆失敗（>2 次）。**禁猜**：don't guess your way through。
...
| 「verify pass 就 commit」「多 task 一個 commit」 | verify pass 只是 commit 前提；一 commit 一 task，違反 rules.md「一 commit 一邏輯改變」|
| 「fail 多 retry 一次過了就好」 | 不靜默 retry；走 §Task fail 處置 |
