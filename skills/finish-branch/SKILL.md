...
- **Hook**：plugin 的 `hooks/guard.mjs`（branch-safety 段；PreToolUse 擋 Write / Edit / NotebookEdit（NotebookEdit 是 Claude Code 才有）、Codex 的 apply_patch）；命中主分支（`main / master / production / prod / release`）→ 阻擋（Claude Code 是 exit 2 + stderr；Codex 是 stdout JSON deny，見 guard.mjs）
