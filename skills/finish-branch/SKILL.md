...
- **Hook**：plugin 的 `hooks/guard.mjs`（branch-safety 段；PreToolUse 擋 Write / Edit / NotebookEdit（NotebookEdit 是 Claude Code 才有）、Codex 的 apply_patch）；命中主分支（`main / master / production / prod / release`）→ exit 2 阻擋
