...
> ---
>
> **2026-09-08 void 改版後仍以本表為基準，內容刻意不更新。** 下面「技術現況」與 `app.js:NNN`
> 的行號記的是**改版前**的樣子（外部 `styles.css` + `app.js` + d3），那些檔已於
> `docs/void-redesign` 刪除；留著是為了回溯原始實作，不是描述現況。
> 現況看 `docs/reference/design-map.md`（架構與色票）與 `docs/tools/docs-site-contract.mjs`
> 的檔頭契約總表（機械可驗的部分）。
>
> 改版後的實測差異只有兩項，其餘 F1–F22 行為保留：
> - **F13/F14 的節點→文件對映**由手寫的 `NODE_DOCS` 改成 label 撞名的啟發式（撞不到的靠
>   `flow.html` 的 `DOC_OVERRIDES` 補）。改版前 29 個對映全數保留、另有 12 個節點新得到文件；
>   契約 C6 拿改版前那份當基準線把關。
> - **響應式**由 1080px / 860px 兩條 `@media` 改成 `clamp()` 與 JS 量測，全站零 `@media`。
>   舊 C15 因此沒有等價物，窄視窗的浮層重疊改由 e2e 把關。
