...
| 11 | **「字串存在」斷言鎖不住位置** —— 把關鍵句搬到描述性段落就繞過（實測 C3 可被攻擊版繞過） | 需要綁定位置時用 `awk` 抽出區段再 grep，不要全檔搜 |
...
```
# Expected: FAIL —— 檔案不存在，14 條全 MISS
# 組成：存在 1 ＋ 下限 1 ＋ 五類標題 1 ＋ 上游五類 5 ＋ 新增五項 5 ＋ 數字綁定 1
```
...
```
# Expected: FAIL —— 12 條：檔案不存在 1 ＋ 5 情境 ＋ 表結構 1 ＋ 力度尺 1 ＋ 混合情境 1 ＋ 仲裁 1 ＋ 文體邊界 1 ＋ 守衛 1
# 註：Task 3、5 的 Expected 都曾漏算最前面那條「檔案不存在」——寫 Step 2 預期時只數內容類斷言是通病
```
...
```
# Expected: FAIL —— 檔案不存在 1 ＋ 5 情境 ＋ 成套 1 ＋ 上限 1 ＋ 保護示範 1 ＋ 守衛 1 = 10 條
...
# 【C3 強化】v2.2 原本用「字串存在」，實測仍可繞過：把綁定句搬到描述性備註、
# §使用契約 第 5 步改成無條件，兩條斷言照樣全綠而自動路徑會卡住 verify-done。
# 改成區段錨定 —— 綁定句必須落在 §使用契約 內（對攻擊版實測變紅）
seg=$(awk '/^## 使用契約/{c=1} c{print} /^---$/{if(c)exit}' "$f")
echo "$seg" | grep -qF "四選項只適用 \`使用者主動呼叫\` 這條路徑" || { echo "MISS(C3): 綁定句不在 §使用契約 內"; ok=0; }
echo "$seg" | grep -qF "被 verify-done 自動載入時不問、不等、直接把清單寫進 verify 結果" || { echo "MISS(C3): 自動載入路徑不在 §使用契約 內"; ok=0; }
# 【衝突 1】四選項 ＋ 推薦 ＋ Other
