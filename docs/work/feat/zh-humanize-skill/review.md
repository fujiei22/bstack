...
# 第二輪：v2.1 的斷言 review（單視角）

> 標的: `plan.md` v2.1 | 日期: 2026-09-02 | 5 Critical / 6 Major

**只派一個視角**（斷言一致性，工作區還熱著）。前三個視角的發現已吸收，且 K11 的 delta 不大幅改變它們的守備範圍。

## 一句話

**v1 抓到的 15 條大部分真的修好了**（Task 2/3/4/8 逐條實跑確認），剩下的問題**全部集中在這一輪新寫的三個地方**：Task 9（全新 task）、Task 7 的兩條路徑（全新機制）、Task 5/6 的錨定（這輪才改）。

**新增的斷言沒有經過「造出內容再跑」這一關，跟 v1 的失敗模式一模一樣**——只是範圍小很多。

## 主 session 實測確認的五條 Critical

| # | 問題 | 實測 |
|---|---|---|
| C1 | Task 9 永遠紅——反向斷言禁掉的 token（`blocker`）出現在 Step 3 自己要求寫的解釋句裡 | 造出改好的 verify-done → `MISS: 對外文字複查不得升 blocker` |
| C2 | **主 session 造成的退步**。上游 `scenes.md` 的情境是 **H2 標題**不是表格列，`^\| ` 錨定命中 **0/5**；v1 的裸 grep 是 3/5 | 修「誤紅」把「漏抓」修壞了 |
| C3 | 可造出「兩條路徑混在一起」但 37 條斷言全綠的版本——§兩條路徑 表完整、§使用契約 第 6 步卻無條件走四選項 | K11 四條斷言全是「字串存在」，沒有一條鎖「綁定」 |
| C4 | 七個關鍵詞漏掉孿生後門條款。上游 `:65-68` 含七詞的 **0 個** | 守住「環境偵測」那一半，漏掉「自由文字當 gate」那一半 |
| C5 | V11 排除清單少 `verify-done/SKILL.md`，Task 9 做完 Task 10 永遠紅 | spec 寫三個、plan 只排除兩個 |

## v2.2 的處置與**逐條實跑驗證**

| 修法 | 驗證方式 | 結果 |
|---|---|---|
| C2 錨定改 `^## ` | 對上游 `scenes.md` 實跑 | **5/5**（v2.1 0/5、v1 3/5） |
| M4 補齊八節 | 對上游 `examples.md` 實跑 | **8/8** |
| C1 改鎖正向措辭 | 造出 Step 3 內容再跑 | PASS |
| C3 加綁定斷言 | runtime backtick 測試 | 兩條都命中 |
| C4 加三條後門 pattern | 查上游命中數 | 各 1 次，**非空砲** |
| M1 guard 鎖完整掛載行 | 查該行在 verify-done 唯一性 | 1 次，鎖得住 |
| 全檔 bash 語法 | `bash -n` 逐塊 | **11 塊全過** |

**驗證紀律 7 → 10 條**，新增三條全部來自這一輪。

## 這兩輪真正的教訓

不是「要多 review 幾輪」，是**主 session 每次寫新斷言都跳過「造出內容再跑」這一步**。第一輪 5 條壞、第二輪 5 條壞，兩次都是新寫的部分，兩次都是同一個原因。v2.2 是第一次在交出去之前自己做完這一步。
