...
for (const f of readdirSync(join(G, 'out')).filter((x) => x.endsWith('.md')).sort()) {
  ...
  let guard; try { execSync(`node "${join(G, 'slim-guard-v2.mjs')}" check "${join(G, 'baseline-4de4e83.json')}" --only ${guardName} --src "${src}"`, { encoding: 'utf8' }); guard = 'PASS'; } catch (e) { guard = (e.stdout || '').split('\n').filter((l) => /^\s{2}\S/.test(l)).map((l) => l.trim()).join(' ／ ') || 'FAIL'; }
